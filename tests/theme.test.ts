...
describe("emitters", () => {
  ...
  it("keeps semantic tokens when source ramp names collide with roles or suffixes", () => {
    const named = ok(theme([
      { hex: "#000000", name: "Accent" },
      { hex: "#ffffff", name: "Accent-2" },
      { hex: "#eeeeee", name: "Accent" },
    ]));
    const tokens = toDesignTokens(named) as { color: Record<string, any> };
    expect(tokens.color.accent.$value.hex).toBe(named.colors.accent.hex);
    expect(tokens.color["accent-2"]["50"].$value).toBeDefined();
    expect(tokens.color["accent-2-2"]["50"].$value).toBeDefined();
    expect(tokens.color["accent-3"]["50"].$value).toBeDefined();
    expect(toCssVariables(named)).toContain("--color-accent-3-50:");
    expect(toTailwindTheme(named)).toContain("--color-accent-2-2-50:");

    for (const [roleName, key, role] of [
      ["Background", "background", "background"],
      ["Surface", "surface", "surface"],
      ["Text", "text", "text"],
      ["Text Muted", "text-muted", "mutedText"],
      ["On Accent", "on-accent", "onAccent"],
    ] as const) {
      const t = ok(theme([{ hex: "#000000", name: roleName }, { hex: "#ffffff", name: "Paper" }]));
      const color = (toDesignTokens(t) as { color: Record<string, any> }).color;
      expect(color[key].$value.hex).toBe(t.colors[role].hex);
      expect(color[`${key}-2`]["50"].$value).toBeDefined();
    }
  });

  it("keeps untrusted swatch names inside CSS comments in both emitters", () => {
    const unsafe = "Ink */\nbody { display: none }\n/*";
    const named = ok(theme([{ hex: "#000000", name: unsafe }, { hex: "#ffffff", name: "Paper" }]));
    expect(named.source[0]?.name).toBe(unsafe);
    for (const css of [toCssVariables(named), toTailwindTheme(named)]) {
      expect(css).toMatch(/^\/\* [^\n]* Generated by @thesnug\/color-picker\. \*\/\n/);
      expect(css.match(/\*\//g)).toHaveLength(1);
      expect(css).not.toContain("*/\nbody");
      expect(css).toContain("--color-background:");
      expect(css).toContain("--color-ink-body-display-none-50:");
    }
  });
});
