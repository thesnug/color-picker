...
the palette and the schema limits each element's hex to it), cached by hash and
palette next to the fingerprints. The pass runs through the Codex CLI on the
