...
```ts
import { acceptedColor, describeToColor } from "@thesnug/color-picker/jev";
...
acceptedColor(shirt); // the color to act on, or undefined when Jev was unsure
```
...
- Anything else is one cached Choice request over every color in the set plus a
  ...
  reported them.
- `acceptedColor` returns the color to act on: the nearest one for a name
  match, or Jev's top match when its probability reaches `DESCRIBE_COLOR_ACCEPT`
  (0.35) and `none` did not win. Otherwise it returns `undefined`; show
  `matches` as suggestions instead.
- `npm run jev:describe:evaluate` reports top-1 and top-3 accuracy on the labeled
  phrases in `tests/fixtures/jev/describe-color.json` against the live API, and
  `npm run jev:evaluate` measures every Jev threshold (see "Evaluating Jev
  thresholds"). Both need a key and never run in CI.
...
dropped[0].reasons.at(-1);        // "Implausible: the strawberry body as … (plausibility 0.08, below 0.8)."
```
...
- A plan is `implausible` when its lowest swap is below `PLAUSIBILITY_THRESHOLD`
  (0.8, set by the threshold evaluation; Jev's answers for swaps run high).
  Implausible plans are dropped and the next garments fill their places; pass
  `includeImplausible` to keep them, marked. `vetRecolorPlans` vets plans you already have and drops
  nothing.
