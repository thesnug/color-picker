...
import type { InkColor, ProductPalette } from "../palettes.js";
import type { ProductColorRecommendation } from "../recommend.js";
import { lightnessWord } from "./describe.js";
import { ask, type AskOptions, type EntryType, JevUnavailableError, type ScoreResponse } from "./index.js";
...
export const MOOD_LEVELS = ["clashes", "neutral", "complements", "elevates"] as const;
...
const JUDGE_NOTE =
  "Judge only how the colors fit what the design depicts and the feeling it gives. " +
...
export interface MoodDesign {
  palette: readonly { hex: string; share: number; element?: string }[];
 ...
 */
export async function rerankByMood<C extends MoodCandidate>(
  candidates: readonly C[],
  design: MoodDesign,
  options: RerankByMoodOptions = {},
): Promise<MoodRerank<C>> {
  const top = options.top ?? MOOD_TOP;
  const weight = options.weight ?? MOOD_WEIGHT;
  if (!(Number.isInteger(top) && top > 0)) throw new RangeError(`top must be a positive integer, got ${top}`);
  checkWeight(weight);

  const shortlist = candidates.slice(0, top);
  const rest = candidates.slice(top);
  const deterministic = (note: string): MoodRerank<C> => ({
    candidates: [
  ...
  if (shortlist.length === 0) return deterministic("There were no candidates to re-rank.");
  if (!design.description) {
    return deterministic(
      "Mood re-ranking needs a vision description of the design (describeDesign), so the order is the deterministic one.",
    );
  }

  const state = moodState(design);
  const questions = moodQuestions(shortlist);
  const askOptions: AskOptions = { version: MOOD_VERSION };
  ...
  const ranked = shortlist.map((c, i) => {
    const answer = result.answers[moodCandidateId(c)] as ScoreResponse | undefined;
 ...
 */
export function combineMood<C>(candidates: readonly MoodRanked<C>[], weight: number = MOOD_WEIGHT): MoodRanked<C>[] {
  checkWeight(weight);
  ...
  shortlist.sort(
    (a, b) => b.combinedScore! - a.combinedScore! || (b.deterministicScore ?? 0) - (a.deterministicScore ?? 0),
  );
 ...
 */
export function moodCandidateId(candidate: MoodCandidate): string {
  if (!isPalette(candidate)) return `garment:${candidate.color.slug}`;
  const garment = candidate.colors[0].slug;
  if (candidate.source === "book") return `palette:${garment}:book-${candidate.combination.id}`;
  const inks = candidate.colors
    .slice(1)
    .map((c) => (c as InkColor).index)
    .join("-");
...
export function moodState(design: MoodDesign) {
  const description = design.description;
  if (!description) throw new TypeError("moodState needs a design with a vision description.");
  return {
    design: {
...
export function moodQuestions(candidates: readonly MoodCandidate[]) {
  const questions: Record<string, { type: "score"; instructions: EntryType; criteria: typeof LEVEL_DESCRIPTIONS }> =
    {};
  for (const candidate of candidates) {
    const id = moodCandidateId(candidate);
    if (questions[id]) throw new Error(`Two candidates share the question name ${JSON.stringify(id)}.`);
...
function isPalette(candidate: MoodCandidate): candidate is ProductPalette {
  return "source" in candidate && "colors" in candidate;
}

function instructionsFor(candidate: MoodCandidate): EntryType {
...
function combine(deterministic: number | null, mood: number | null, weight: number): number | null {
  if (deterministic === null) return null;
  if (mood === null) return deterministic;
  return round((1 - weight) * deterministic + weight * mood);
}

/** Place in the shortlist as 0 to 1, first highest. */
function shortlistScore(index: number, length: number): number {
  return length === 1 ? 1 : round(1 - index / (length - 1));
}

function checkWeight(weight: number): void {
  if (!(weight >= 0 && weight <= 1)) throw new RangeError(`weight must be from 0 to 1, got ${weight}`);
}
