...
export interface VetResult {
  ...
  skipped?: { reason: VetSkipReason; note: string };
 ...
 */
export async function vetRecolorPlans(
  plans: readonly RecolorPlan[],
  design: VetDesign,
  options: VetOptions = {},
): Promise<VetResult> {
  ...
  const askOptions: AskOptions = { version: VET_RECOLOR_VERSION };
  ...
  let skipped: VetResult["skipped"];
  let model: string | undefined;
  const results = await Promise.all(
    plans.map(async (plan): Promise<VettedPlan> => {
      const questions = vetQuestions(plan.mapping, description);
      if (Object.keys(questions).length === 0) return judged(plan, plan.mapping.map(() => 1), threshold);
      try {
        const { answers, model: answeredBy } = await ask(state, questions, askOptions);
        model ??= answeredBy;
        const yes = plan.mapping.map((_, i) => {
          const answer = answers[questionName(i)] as NoulResponse | undefined;
  ...
  const result: VetResult = { plans: results, vetted: results.filter((p) => p.plausibility !== null).length };
 ...
 */
export async function vettedRecolorPlans(
  design: DesignSummary & VetDesign,
  options: VettedRecolorOptions = {},
): Promise<VettedRecolorResult> {
  ...
  const kept: VettedPlan[] = [];
  const dropped: VettedPlan[] = [];
  let vetted = 0;
  let skipped: VetResult["skipped"];
  let model: string | undefined;
  for (let start = 0; start < ranked.length && kept.length < n; start += n) {
    ...
    for (const plan of batch.plans) {
      if (plan.implausible) dropped.push(plan);
      else if (kept.length < n) kept.push(plan);
    }
  }
  const result: VettedRecolorResult = { plans: kept, vetted, dropped };
  if (skipped) result.skipped = skipped;
  if (model !== undefined) result.model = model;
  return result;
}
 ...
 */
export function vetState(description: NonNullable<VetDesign["description"]>) {
  return {
...
const questionName = (index: number) => `swap_${index}`;

/** Whether a mapping entry changes its color enough to ask about. */
export function isChange(entry: Pick<RecolorMapping, "from" | "to">): boolean {
...
export function swapQuestion(
  entry: Pick<RecolorMapping, "from" | "to">,
  description: Pick<DesignDescription, "elements">,
): string {
  ...
  if (entry.from.element) return `If the ${entry.from.element} changes from ${from} to ${to}, ${tail}`;
...
function fromWords(entry: Pick<RecolorMapping, "from">, description: Pick<DesignDescription, "elements">): string {
  const hex = entry.from.hex.toLowerCase();
  const words = [...new Set(description.elements.filter((e) => e.hex === hex).map((e) => e.color))];
  if (words.length) return words.join(" and ");
...
function unvetted(plan: RecolorPlan): VettedPlan {
  return { ...plan, mapping: plan.mapping.map((m) => ({ ...m, plausibility: null })), plausibility: null, implausible: false };
}

function judged(plan: RecolorPlan, yes: readonly number[], threshold: number): VettedPlan {
  ...
  if (implausible) {
    for (const m of mapping.filter((m) => m.plausibility < threshold)) {
      ...
      reasons.push(
        `Implausible: ${subject} as ${m.to.name} ${m.to.hex} may not read as the same subject ` +
