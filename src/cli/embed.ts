 ...
 */
export async function embedImages(markup: string, options: EmbedOptions = {}): Promise<string> {
  if (process.env[EMBED_ENV] === "0") return markup;
  const urls = [...new Set([...markup.matchAll(IMAGE_HREF)].map((m) => unescapeXml(m[2]!)))];
  if (urls.length === 0) return markup;
  const inlined = new Map(await Promise.all(urls.map(async (url) => [url, await dataUri(url, options)] as const)));
  return markup.replace(IMAGE_HREF, (whole, open: string, href: string, close: string) => {
    const uri = inlined.get(unescapeXml(href));
    return uri ? `${open}${uri}${close}` : whole;
  });
}

function dataUri(url: string, options: EmbedOptions): Promise<string | undefined> {
...
async function download(url: string, fetchImpl: typeof fetch): Promise<string | undefined> {
  try {
