...
import { applyRecolor, fingerprint } from "../fingerprint/index.js";
...
export function palettesView({ name, limit, size, product: productId }) {
    let product;
    try {
    ...
    const heading = (p) => `${productPaletteTitle(p)} · via ${p.equivalent.name} · contrast ${fmt(p.contrast)} (lowest ${fmt(p.minContrast)})`;
...
export async function recommendView({ file, bytes, n, product: productId }) {
    let product;
    try {
        product = loadProduct(productId ?? loadProductIndex().default);
    }
    ...
    const heading = (i) => `${i + 1}. ${picks[i].color.name} · score ${fmt(picks[i].score, 3)}`;
...
export async function recolorView({ file, bytes, n, product: productId, apply }) {
    let product;
    try {
        product = loadProduct(productId ?? loadProductIndex().default);
    }
