...
export function toolDefinitions(store = new ResultStore()) {
    const reply = (view, extra = {}) => json({ resultId: store.add(view), ...view.json, ...extra, svg: viewSvg(view) });
    const tools = [
        {
            name: "nearest_colors",
        ...
        },
        {
            ...
            handler: async ({ name, product: productId, size, limit }) => reply(palettesView({
                name: name,
                limit: limit ?? DEFAULT_COMBOS_LIMIT,
                ...(size !== undefined && { size: size }),
 ...
 */
export function jevStatus(option) {
