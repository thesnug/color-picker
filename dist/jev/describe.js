 ...
 */
import { distance, normalizeHex, toOklch } from "../color/convert.js";
import { colorFamily } from "../color/family.js";
import { loadColors, loadProduct, loadProductIndex, } from "../data/index.js";
import { nearest, resolveQuery } from "../nearest.js";
 ...
 */
export async function describeToColor(text, options = {}) {
    const trimmed = text.trim();
    if (!trimmed)
        throw new TypeError("describeToColor needs a description.");
    const set = options.set ?? "wada";
    if (set === "wada") {
        const colors = loadColors();
        const byName = wadaByName(trimmed, colors);
        if (byName)
            return { via: "name", text: trimmed, set, ...byName };
    ...
    if (byName)
        return { via: "name", text: trimmed, set, product: product.id, ...byName };
 ...
 */
export function acceptedColor(result, threshold = DESCRIBE_COLOR_ACCEPT) {
    if (result.via === "name")
        return result.matches[0]?.color;
...
export function lightnessWord(hex) {
    const { l } = toOklch(hex);
    if (l < LIGHTNESS_WORD.darkBelow)
        return "dark";
    if (l >= LIGHTNESS_WORD.lightFrom)
        return "light";
    return "mid";
}
/**
 ...
 */
export function describeQuestion(set, product) {
    if (set === "wada")
        return buildQuestion(set, undefined, loadColors(), wadaOption);
    const resolved = resolveProduct(product);
    return buildQuestion(set, resolved, resolved.colors, productOption);
}
function wadaOption(color) {
    const hex = color.hex;
    return { hex, family: colorFamily(hex, color.name), lightness: lightnessWord(hex) };
}
function productOption(color) {
    const c = color;
    const description = { family: c.family };
    if (c.hex) {
        description.hex = c.hex;
        description.lightness = lightnessWord(c.hex);
    }
    if (!c.available)
        description.stocked = false;
    return description;
}
function buildQuestion(set, product, colors, describe) {
    const criteria = {};
    ...
    const scope = set === "wada"
        ? "the colors of Sanzo Wada's A Dictionary of Color Combinations"
        : `the garment colors of the ${product?.name ?? "product"}`;
    return {
        color: {
            type: "choice",
            instructions: `The state is a person's description of a color. Which one of ${scope} does it most likely mean? ` +
...
async function byJev(text, set, product, colors, describe, options) {
    const questions = buildQuestion(set, product, colors, describe);
    const askOptions = { version: DESCRIBE_COLOR_VERSION };
    ...
    const matches = Object.entries(answer.probabilities)
        ...
        .map(([label, probability]) => ({ color: byName.get(label), probability }));
    const result = {
        via: "jev",
        text,
        set,
...
function wadaByName(text, colors) {
...
function productByName(text, product) {
    const exact = findProductColor(product, text);
    const resolved = exact?.hex
...
function productNearest(hex, product) {
