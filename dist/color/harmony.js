 ...
 */
import { reduceChroma, toOklch } from "./convert.js";
/**
...
function rotate(base, degrees) {
    return reduceChroma({ ...base, h: (((base.h + degrees) % 360) + 360) % 360 });
}
function withHues(input, offsets) {
 ...
 */
export function analogous(input, spread = 30) {
 ...
 */
export function ramp(input, options = {}) {
    ...
    for (const [name, value] of [["lightest", lightest], ["darkest", darkest]]) {
        if (!(value >= 0 && value <= 1)) {
