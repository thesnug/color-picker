 ...
 */
import { toOklch } from "./convert.js";
 ...
 * catalogs do.
 */
export function colorFamily(input, name) {
    if (name !== undefined && /^neon\b/i.test(name.trim()))
        return "neon";
    const { l, c, h } = toOklch(input);
    if (c < FAMILY.neutralChroma)
        return "neutral";
    if (l >= FAMILY.paleLightness && c < FAMILY.paleChroma)
        return "neutral";
    if (c < FAMILY.earthChroma && h >= FAMILY.earthHueMin && h < FAMILY.earthHueMax)
        return "earth";
    if (h >= FAMILY.redHueMin || h < FAMILY.redHueMax)
        return "red-pink";
    if (h < FAMILY.orangeHueMax)
        return "orange-yellow";
    if (h < FAMILY.greenHueMax)
        return "green";
    if (h < FAMILY.blueHueMax)
        return "blue";
    return "purple";
}
