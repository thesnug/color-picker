...
export function rgbToHex(rgb) {
    const part = (v) => Math.round(Math.min(255, Math.max(0, v)))
        .toString(16)
        .padStart(2, "0");
...
export function linearToRgb(linear) {
    const enc = (v) => 255 * linearChannelToSrgb(Math.min(1, Math.max(0, v)));
...
export function oklabToLinear({ l: L, a, b }) {
 ...
 */
export function gamutMap(input) {
    ...
    if (deltaE(clipped, oklchToOklab(origin)) < JND)
        return oklabToOklch(clipped);
    let min = 0;
    let max = origin.c;
    let minInGamut = true;
    while (max - min > EPSILON) {
        const current = { ...origin, c: (min + max) / 2 };
 ...
 */
export function reduceChroma(input) {
    const origin = toOklch(input);
    if (origin.l >= 1)
        return { l: 1, c: 0, h: 0 };
    if (origin.l <= 0)
...
export function toRgb(input, options = {}) {
    if (typeof input === "string")
...
export function toHex(input, options = {}) {
    if (typeof input === "string")
