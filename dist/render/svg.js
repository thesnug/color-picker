...
import { NOT_STOCKED, escapeXml, fmt, readableTextColor, swatchHex, wrapText } from "./shared.js";
...
function titleText(title) {
    return (`<text x="0" y="18" font-size="15" font-weight="600" fill="currentColor">` +
...
function unstockedOutline(x, y, width, height, ink, rx = 0) {
    return (`<rect x="${x + 3}" y="${y + 3}" width="${width - 6}" height="${height - 6}" rx="${rx}" ` +
 ...
 */
function tile(x, y, width, height, swatch) {
    const hex = swatchHex(swatch);
    ...
    if (swatch.note) {
        for (const line of wrapText(swatch.note, maxChars, 1))
            lines.push({ text: line, size: 11 });
    }
    const unstocked = swatch.available === false;
    if (unstocked)
        lines.push({ text: NOT_STOCKED, size: 11, weight: 600 });
    ...
    return (`<g><title>${escapeXml(describe(swatch))}</title>` +
...
export function pairContrast(left, right) {
...
function contrastBadge(cx, y, left, right) {
    ...
    const height = 22;
    const detail = `${describe(left)} and ${describe(right)}: WCAG ${fmt(wcag)}:1, ` +
        `APCA Lc ${fmt(apca, 1)} (left as text on right)`;
    return (`<g><title>${escapeXml(detail)}</title>` +
 ...
 */
export function renderCombination(combo, options = {}) {
 ...
 */
export function renderSwatchGrid(colors, options = {}) {
    const tileWidth = options.tileWidth ?? 128;
    ...
    if (colors.length === 0) {
        ...
        parts.push(`<text x="0" y="${top + 18}" font-size="13" fill="currentColor">No colors</text>`);
 ...
 */
export function renderSwatchStripSvg(hexes, options = {}) {
    ...
    const tiles = hexes
        ...
        return (`<rect x="${i * tileWidth}" y="0" width="${tileWidth}" height="${tileHeight}" fill="${safe}">` +
            `<title>${safe}</title></rect>`);
    })
        .join("");
    return (`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${tileHeight}" ` +
 ...
 * is plain.
 */
export function renderProductCard(productColor, options = {}) {
    ...
    parts.push(`<rect x="0" y="${top}" width="${size}" height="${size}" rx="8" fill="${hex}" stroke="#00000022"/>`);
    const url = productColor.image?.url;
    if (url) {
        const clipId = `garment-${hashId(`${url}|${hex}`)}`;
        parts.push(`<clipPath id="${clipId}"><rect x="0" y="${top}" width="${size}" height="${size}" rx="8"/></clipPath>` +
    ...
    parts.push(`<text x="0" y="${top + size + 20}" font-size="13" font-weight="600" fill="currentColor">` +
        `${escapeXml(name)}</text>` +
    ...
    rows.forEach(({ chip, before }, i) => {
        ...
        parts.push(`<g><title>${escapeXml(tooltip)}</title>` +
            (before
