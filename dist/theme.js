 ...
 */
import { apcaLevel, label, roles, toSwatches, wcagLevel, } from "./accessibility.js";
import { apcaContrast, contrastRatio } from "./color/contrast.js";
import { oklabToHex, oklchToHex, reduceChroma, toHex, toOklab, toOklch, } from "./color/convert.js";
import { loadAccessibility } from "./data/index.js";
/** The roles a theme fills, in emit order. */
export const THEME_ROLES = ["background", "surface", "text", "mutedText", "accent", "onAccent"];
/**
 ...
 * lightness there.
 */
const SURFACE_CONTRAST = { light: 1.08, dark: 1.2 };
 ...
 * exists for the mode.
 */
export function theme(combination, options = {}) {
    ...
    const surface = deriveSurface(background, mode, (hex) => contrastRatio(text.hex, hex) >= textMin && contrastRatio(accent.hex, hex) >= uiMin);
    ...
    const mutedText = deriveMuted(text, background, (hex) => contrastRatio(hex, background.hex) >= textMin && contrastRatio(hex, surface.hex) >= textMin);
    ...
    const colors = { background, surface, text, mutedText, accent, onAccent };
    ...
    if (failing.length > 0) {
        ...
        throw new Error(`theme: pairing ${failing.map((p) => `${p.foreground} on ${p.background}`).join(", ")} fails WCAG AA`);
    }
    return { ok: true, mode, colors, source: toSwatches(combination), pairings, reasons };
}
function deriveSurface(background, mode, acceptable) {
    ...
    for (let shift = SURFACE_STEP; shift <= 0.25; shift += SURFACE_STEP) {
        ...
        if (hex !== background.hex)
            best = hex;
        if (contrastRatio(hex, background.hex) >= target || l === 0 || l === 1)
            break;
    }
    return best
...
function deriveMuted(text, background, acceptable) {
...
function pairing(foreground, background, colors, context, options) {
...
export function describeRole(role, color) {
    return `${role}: ${label(color)}${color.derived ? ` — ${color.derived}` : ""}`;
}
// ---------------------------------------------------------------------------
// Ramps
/** The Tailwind step names for an eleven-step ramp. */
export const RAMP_NAMES = ["50", "100", "200", "300", "400", "500", "600", "700", "800", "900", "950"];
/**
 ...
 */
export function namedRamp(color, options = {}) {
    ...
    const swatch = typeof color === "object" && !Array.isArray(color) && "hex" in color
    ...
    lightness.forEach((l, i) => {
        if (Math.abs(l - origin.l) < Math.abs(lightness[nearest] - origin.l))
            nearest = i;
    });
    ...
    const steps = names.map((name, i) => {
        if (i === anchor)
            return { name, hex: swatch.hex, oklch: origin };
        const l = lightness[i];
...
function rampNames(count) {
    if (count === RAMP_NAMES.length)
        return [...RAMP_NAMES];
...
function properties(theme, options) {
    const props = THEME_ROLES.map((role) => [`color-${ROLE_NAMES[role]}`, theme.colors[role].hex]);
    if (options.ramps ?? true) {
        for (const [key, r] of sourceRamps(theme)) {
            for (const step of r.steps)
                props.push([`color-${key}-${step.name}`, step.hex]);
        }
    }
    return props;
}
 ...
 */
export function toCssVariables(theme, options = {}) {
 ...
 */
export function toTailwindTheme(theme, options = {}) {
    const lines = properties(theme, options).map(([name, hex]) => `  --${name}: ${hex};`);
    return `${header(theme)}\n@import "tailwindcss";\n\n@theme {\n${lines.join("\n")}\n}\n`;
}
...
function colorToken(hex, description) {
    const components = [1, 3, 5].map((i) => round(parseInt(hex.slice(i, i + 2), 16) / 255, 4));
    return {
 ...
 */
export function toDesignTokens(theme, options = {}) {
    const color = {
        $type: "color",
    ...
    if (options.ramps ?? true) {
        for (const [key, r] of sourceRamps(theme)) {
            const group = r.color.name ? { $description: r.color.name } : {};
            for (const step of r.steps)
                group[step.name] = colorToken(step.hex);
            color[key] = group;
        }
    }
    return {
